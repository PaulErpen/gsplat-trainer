...
from gsplat_trainer.train.strategy.base import Strategy
from gsplat_trainer.train.strategy.ops import duplicate, remove, reset_opa, split
...
class DefaultStrategy(Strategy):
    ...
    def _grow_gs(
        ...
        diff = self.cap_max - params["means"].shape[0]
        top_k_highest_grads = get_top_k_indices(grads, diff)
        mask_top = torch.zeros(len(grads), dtype=torch.bool, device=device)
        mask_top[top_k_highest_grads] = True

        is_grad_high = (grads > self.grow_grad2d) & mask_top
